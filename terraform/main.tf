...
resource "azurerm_resource_group" "main" {
  name     = "rg-agentbridge-${var.environment}-kyc-anomaly-detector-to-reduce-fraud-los"
  location = var.region
  tags = {
...
resource "azurerm_virtual_network" "main" {
  name                = "vnet-agentbridge-kyc-anomaly-detector-to-reduce-fraud-los"
...
resource "azurerm_subnet" "main" {
  name                 = "snet-agentbridge-kyc-anomaly-detector-to-reduce-fraud-los"
  resource_group_name  = azurerm_resource_group.main.name
...
resource "azurerm_network_security_group" "main" {
  name                = "nsg-agentbridge-kyc-anomaly-detector-to-reduce-fraud-los"
  location            = azurerm_resource_group.main.location
  resource_group_name = azurerm_resource_group.main.name
...
resource "azurerm_public_ip" "main" {
  ...
  name                = "pip-agentbridge-kyc-anomaly-detector-to-reduce-fraud-los"
  location            = azurerm_resource_group.main.location
  resource_group_name = azurerm_resource_group.main.name
...
resource "azurerm_network_interface" "main" {
  name                = "nic-agentbridge-kyc-anomaly-detector-to-reduce-fraud-los"
  location            = azurerm_resource_group.main.location
  resource_group_name = azurerm_resource_group.main.name
...
resource "azurerm_linux_virtual_machine" "main" {
  name                  = "vm-agentbridge-kyc-anomaly-detector-to-reduce-fraud-los"
  resource_group_name   = azurerm_resource_group.main.name
  location              = azurerm_resource_group.main.location
...
resource "azurerm_managed_disk" "data" {
  ...
  name                 = "disk-agentbridge-kyc-anomaly-detector-to-reduce-fraud-los-data"
  location             = azurerm_resource_group.main.location
  resource_group_name  = azurerm_resource_group.main.name
  storage_account_type = "Premium_LRS"
  create_option        = "Empty"
...
resource "azurerm_cognitive_account" "ai_services" {
  ...
  name                  = "agentbridge-kyc-anomaly-detector-to-reduce-fraud-los-ai"
  location              = azurerm_resource_group.main.location
  resource_group_name   = azurerm_resource_group.main.name
  kind                  = "AIServices"
  sku_name              = "S0"
  custom_subdomain_name = "agentbridge-kyc-anomaly-detector-to-reduce-fraud-los-ai"
...
resource "azurerm_user_assigned_identity" "vm_foundry" {
  count               = var.enable_foundry ? 1 : 0
  name                = "agentbridge-kyc-anomaly-detector-to-reduce-fraud-los-vm-identity"
  location            = azurerm_resource_group.main.location
  resource_group_name = azurerm_resource_group.main.name
}
...
resource "null_resource" "foundry_agents" {
  ...
  provisioner "local-exec" {
    interpreter = ["bash", "-c"]
    command     = <<-EOT
      ENDPOINT="https://agentbridge-kyc-anomaly-detector-to-reduce-fraud-los-ai.services.ai.azure.com"
